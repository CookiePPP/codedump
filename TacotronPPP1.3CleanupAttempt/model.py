...
from utils import to_gpu, get_mask_from_lengths, dropout_frame, get_mask_3d
#from modules import GST # mellotron GST implementation (seems to have batch problems)
...
class Encoder(nn.Module):
    ...
    def forward(self, text, text_lengths, speaker_ids=None):
        ...
        text = nn.utils.rnn.pack_padded_sequence(
            text, text_lengths, batch_first=True)
...
class Decoder(nn.Module):
    def __init__(self, hparams):
        ...
        if self.AttRNN_hidden_dropout_type == 'dropout':
            ...
            self.attention_rnn = LSTMCellWithZoneout(
                attention_rnn_in_dim, # input_size
                hparams.attention_rnn_dim, zoneout_prob=self.p_DecRNN_hidden_dropout) # hidden_size, zoneout)
        ...
        if self.attention_type == 0:
            ...
            raise NotImplementedException("attention_type invalid, valid values are... 0 and 1")
        ...
        if self.DecRNN_hidden_dropout_type == 'dropout':
            ...
            self.decoder_rnn = LSTMCellWithZoneout(
                hparams.attention_rnn_dim + self.encoder_dim, # input_size
                hparams.decoder_rnn_dim, 1, zoneout_prob=self.p_DecRNN_hidden_dropout) # hidden_size, zoneout)
        ...
        self.gate_layer = LinearNorm(
            hparams.decoder_rnn_dim + self.encoder_dim, 1,
    ...
    def initialize_decoder_states(self, memory, mask, preserve=None):
        """ Initializes attention rnn states, decoder rnn states, attention
        weights, attention cumulative weights, attention context, stores memory
        and stores processed memory
        PARAMS
        ------
        memory: Encoder outputs [B, enc_time, enc_dim]
        mask: Mask for padded data if training, expects None for inference
        preserve: Batch shape bool tensor of decoder states to preserve [B]
        """
        B = memory.size(0)
        ...
        if hasattr(self, 'saved_attention_weights') and preserve is not None:
            ...
            self.attention_weights[:, :COMMON_ENCODE] = self.saved_attention_weights[:, :COMMON_ENCODE]
    ...
    def forward(self, memory, decoder_inputs, memory_lengths, f0s=None, preserve_decoder=None, teacher_force_till=None, p_teacher_forcing=None):
        ...
        decoder_inputs = torch.cat((decoder_input, decoder_inputs), dim=0) # add blank starting frame(s)
    ...
    def inference(self, memory, f0s=None):
        ...
        sig_max_gates = torch.zeros(decoder_input.size(0), device=decoder_input.device, dtype=decoder_input.dtype)
        ...
        for i in range(self.max_decoder_steps):
            ...
            if self.attention_type == 1 and self.num_att_mixtures == 1:## stop when the attention location is out of the encoder_outputs
                ...
                if i > 7: # model has very *interesting* starting predictions
                    sig_max_gates = torch.max(torch.sigmoid(gate_output), sig_max_gates)
                if sig_max_gates.min() > self.gate_threshold:
...
class Tacotron2(nn.Module):
    ...
    def parse_batch(self, batch):
        text_padded, text_lengths, mel_padded, gate_padded, \
            output_lengths, speaker_ids, torchmoji_hidden, preserve_decoder_states, align_padded, f0_padded = batch
        ...
        return (
            (text_padded, text_lengths, mel_padded, max_len, output_lengths, speaker_ids, torchmoji_hidden, preserve_decoder_states, align_padded, f0_padded),
            (mel_padded, gate_padded, output_lengths, align_padded))
    ...
    def forward(self, inputs, teacher_force_till=None, p_teacher_forcing=None, drop_frame_rate=None):
        ...
        mel_outputs, gate_outputs, alignments = self.decoder(
            encoder_outputs, gt_mels, memory_lengths=text_lengths, f0s=f0_padded, preserve_decoder=preserve_decoder_states, teacher_force_till=teacher_force_till, p_teacher_forcing=p_teacher_forcing)
    ...
    def inference(self, text, speaker_ids, f0s=None, style_input=None, style_mode=None):
        
        embedded_text = self.embedding(text).transpose(1, 2) # [B, embed, sequence]
        encoder_outputs = self.encoder.inference(embedded_text, speaker_ids=speaker_ids) # [B, time, encoder_out]
        ...
        if hasattr(self, "speaker_embedding"):
            encoder_outputs = torch.cat((encoder_outputs, self.speaker_embedding(speaker_ids)[:, None].repeat(1, encoder_outputs.size(1), 1)), dim=2) # [batch, time, encoder_out]
