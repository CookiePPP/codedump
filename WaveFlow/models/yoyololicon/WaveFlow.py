...
from models.tensor_utils import permute_height
...
#    n_early_size, memory_efficient, spect_scaling, upsample_mode, WN_config, win_length, hop_length):
    ...
    def forward(self, spect, audio, speaker_ids=None): # optional spect input
        ...
        audio, spect = self.squeeze(audio, spect) # [B, mels, T//n_group]
        
        output_audio = []
        split_sections = [self.n_early_size, self.n_group]
        for k, affine_coup in enumerate(self.WN):
            if k % self.n_early_every == 0 and k > 0:
            ...
            audio = permute_height(audio, reverse=True) # (reverse=False, bipart=False, shift=False, inverse_shift=False)
        
        assert split_sections[1] == self.z_split_sizes[-1]
        output_audio.append(audio)
        return torch.cat(output_audio, 2).transpose(2, 3).contiguous().view(audio.shape[0], -1), logdet
    ...
    def inverse(self, z, spect, speaker_ids=None):
        ...
        assert z.size(3) <= spect.size(3)
        spect = spect[..., :z.size(3)]
        ...
        *remained_z, z = remained_z
        ...
        for k, affine_coup in zip(range(self.n_flows - 1, -1, -1), self.WN[::-1]):
            
            z = permute_height(z, reverse=True) # (reverse=False, bipart=False, shift=False, inverse_shift=False)
        ...
        z = z.transpose(2, 3).contiguous().view(batch_dim, -1)
        return z, logdet
