...
class TacotronSTFT(torch.nn.Module):
    ...
    def mel_spectrogram(self, y, ref_level_db = 20, magnitude_power=1.5):
        ...
        magnitudes, phases = self.stft_fn.transform(y) # get magnitudes at each (overlapped) window [B, T] ->  # [B, filter_length, T//hop_length + 1]
        magnitudes = magnitudes.data
        mel_output = torch.matmul(self.mel_basis, magnitudes) # put into mel bins(?)
        mel_output = self.spectral_normalize(mel_output) # convert magnitudes to log scale
