...
class Invertible1x1Conv(nn.Conv2d):
    ...
    def __init__(self, c, hparams):
        super().__init__(1, c, (c, 1), bias=False) # init as nn.Conv1d(c, c, kernel_size=1, stride=1) 
    ...
    def inverse(self, audio_out):
        if hasattr(self, 'efficient_inverse'):
            ...
            *_, n_of_groups = audio_out.shape
            ...
            z = F.conv2d(audio_out, weight.inverse().view(1,*weight.shape,1))
            return z, log_det_W
    ...
    def backward(ctx, z_grad, log_det_W_grad):
        ...
        *_, n_of_groups = audio_out.shape
        ...
        with torch.no_grad():
            ...
            z[:] = F.conv2d(audio_out, inv_weight.view(1,*inv_weight.shape,1))
            
            dx = F.conv2d(z_grad, weight[..., 0].t().view(1,*weight.shape,1))
...
class InvConv1x1Func(Function):
    ...
    def forward(ctx, z, inv_weight):
        with torch.no_grad():
            ...
            *_, n_of_groups = z.shape
            ...
            audio_out = F.conv2d(z, sqr_inv_weight.inverse().view(1,*sqr_inv_weight.shape,1))
    ...
    def backward(ctx, z_grad, log_det_W_grad):
        ...
        *_, n_of_groups = audio_out.shape
        ...
        with torch.no_grad():
            z.storage().resize_(reduce(mul, audio_out.shape))
            z[:] = F.conv2d(audio_out, inv_weight)
            
            inv_weight = inv_weight.squeeze()
            weight_T = inv_weight.inverse().t()
            dx = F.conv2d(z_grad, weight_T.view(1,*weight_T.shape,1))
